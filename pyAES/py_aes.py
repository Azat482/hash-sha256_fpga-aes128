...
class SerialAES(BaseAESOperations):
    ...
    def AES_cipher(self, input, key):
        round_keys = self.KeyExpansion(key)
        
        blocks_length = len(input)
        while blocks_length % (N_B * 4) != 0: blocks_length += 1
        blocks = np.zeros(blocks_length, dtype=np.uint8)
        for i in range(len(input)): blocks[i] = np.uint8(input[i])
        ...
        while block_index < blocks_length:
            ...
            output_block = self.AES_block_cipher(input_block, round_keys)
            print('Block encryption time: ', time.time()- start_time)
            for column in range(columns):
                for row in range(rows):
    ...
    def AES_decrypter(self, input, key):
        round_keys = self.KeyExpansion(key)
        ...
        while block_index < blocks_length:
            for i in range(N_B * 4):
                ...
                block_index += 1
            start_time = time.time()
            output_block = self.AES_block_decrypter(input_block, round_keys)
            print('Block decryption time: ', time.time() - start_time)
            for column in range(columns):
                for row in range(rows):
                    output[output_index] = output_block[row][column]
                    output_index += 1    
