...
uint64_t AES_cipher(uint8_t* input, uint8_t* output, uint8_t* round_keys, uint64_t input_length, int key_length);     //main function of encrypting, return length obtained cipher data
uint64_t AES_decrypter(uint8_t* input, uint8_t* output, uint8_t* round_keys, uint64_t input_length, int key_length);  //main function of decrypting, return length obtained initial data, it may be a little different, if initial message not multiple 16 bytes
