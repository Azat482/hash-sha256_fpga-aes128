...
void AddInputToState(uint8_t* input, uint8_t state[][N_B], int rows, int columns)
{
    ...
    for(column = 0; column < columns; column++)
    {
        ...
        for(row = 0; row < rows; row++, input_index++)
        {
            ...
            //printf("row: %d, column: %d, input[%d]: %x, item = %x\n", row, column, input_index, input[input_index], state[row][column]);
        }
    }
    return;
}
...
uint64_t AES_cipher(uint8_t* input, uint8_t* output, uint8_t* key, uint64_t input_length, int key_length)
{
    uint8_t* blocks;
    uint8_t* round_keys;
    int Nk = key_length;
    ...
    uint64_t blocks_length = input_length;
    while(blocks_length % (N_B * 4) != 0) blocks_length++; //padding length
    blocks = (uint8_t*)malloc(blocks_length * sizeof(uint8_t));

    for(i = 0; i < input_length; i++)             blocks[i] = input[i];
    for(i = input_length; i < blocks_length; i++) blocks[i] = 0x00;
...
uint64_t AES_decrypter(uint8_t* input, uint8_t* output, uint8_t* key, uint64_t input_length, int key_length)
{
    uint8_t* blocks;
    uint8_t* round_keys;
    int Nk = key_length;
